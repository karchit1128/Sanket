...
class GestureDetector:
    ...
    def _detect_alphabets(self, hand_1, handedness_1, hand_2, handedness_2, f1):
        ...
        # Check C vs L
        # L shape requires thumb to be extended (pointing away) and strictly only index extended
        if dom_f['thumb_extended']:
        ...
        # C shape: Index curved, thumb out (but might not trigger strict 'thumb_extended' due to curling inward)
        if not dom_f['fingers_extended']['index'] and not dom_f['fingers_extended']['middle'] and folded_count_dom >= 2:
            index_pip_angle = dom_f.get('index_pip_angle', 0)
            if 80 < index_pip_angle < 150:
                # Ensure thumb is somewhat out from the palm, not tucked in tightly
                if dom_f.get('thumb_pinky_dist', 0) > 0.5:
                    single_gest, single_conf = "C", 0.95

        # I shape: Index finger pointing to cheek/eye (ISLRTC Sign 1)
        if dom_f['fingers_extended']['index'] and folded_count_dom == 3 and not dom_f['thumb_extended']:
            if dom_f.get('index_pip_angle', 180) > 150: # Must be straight
                single_gest, single_conf = "I", 0.98

        # J shape: Index finger vertically hooked (differentiating from C and L)
        if folded_count_dom == 3:
            pts = dom_f['pts']
            # Check if index finger is overall pointing vertically (Tip is vertically separated from MCP, but horizontally aligned)
            dy_mcp_tip = abs(pts[5][1] - pts[8][1])
            dx_mcp_tip = abs(pts[5][0] - pts[8][0])
            if dy_mcp_tip > dx_mcp_tip:
                # Must be curled/hooked (not perfectly straight like L)
                index_pip_angle = dom_f.get('index_pip_angle', 0)
                if index_pip_angle < 168: 
        ...
        # K: Index bent touching straight index (Checked first to prevent E/G/D from stealing it)
        if nondom_fingers['index'] and fold_nondom == 3 and fold_dom == 3:
            # Dominant index must NOT point straight up (to prevent stealing E). 
            # In K, the dominant hand points downwards or horizontally, so tip is below or roughly equal to MCP.
            if pts_dom[8].y > pts_dom[5].y - 0.05:
                # Check thumb tucked (angle < 165) to be robust against 2D projection issues, prevents stealing D
                if dom_f.get('thumb_pip_angle', 180) < 165 or not dom_f['thumb_extended']:
                    if (self._are_hands_touching(pts_dom[6], pts_nondom[6], scale, threshold=T*2.0) or 
                        ...
                        self._are_hands_touching(pts_dom[6], pts_nondom[8], scale, threshold=T*2.0) or
                        self._are_hands_touching(pts_dom[8], pts_nondom[8], scale, threshold=T*2.0)):
                        return "K", 0.98
        ...
        if nondom_fingers['index'] and dom_f['thumb_extended'] and not dom_fingers['middle'] and not dom_fingers['ring'] and not dom_fingers['pinky']:
            if self._are_hands_touching(pts_dom[8], pts_nondom[8], scale, threshold=T, debug_label="D-Index-Tips"):
                if self._are_hands_touching(pts_dom[4], pts_nondom[5], scale, threshold=T*2.5, debug_label="D-Thumb-to-Base") or self._are_hands_touching(pts_dom[4], pts_nondom[6], scale, threshold=T*2.5, debug_label="D-Thumb-to-Base"):
                    return "D", 0.98
